...
def test_get_dialogue_response_invalid_index():
    # prepare
    dialogue_options = [{"question": "Question 1", "response": "Answer 1"}]
    npc = NPC("Guide", "A helpful guide.", dialogue_options)
...
def test_get_dialogue_options_hides_unmet_conditions():
    # prepare - a question gated behind game state that hasn't happened yet
    unlocked = []
    dialogue_options = [
        {"question": "Always available", "response": "Sure."},
        {
            "question": "Locked",
            "response": "Now you know.",
            "condition": lambda: bool(unlocked),
        },
    ]
    npc = NPC("Guide", "A helpful guide.", dialogue_options)

    # call/check - hidden while the condition is false
    assert len(npc.get_dialogue_options()) == 1
    assert npc.get_dialogue_options()[0]["question"] == "Always available"

    # call/check - and unlocked once it becomes true, without rebuilding the NPC
    unlocked.append(1)
    assert len(npc.get_dialogue_options()) == 2
    assert npc.get_dialogue_options()[1]["question"] == "Locked"


def test_get_dialogue_response_indexes_available_options():
    # prepare - a hidden option ahead of a visible one, so a response looked up
    # by the position shown in the menu would be wrong if it indexed the raw list
    dialogue_options = [
        {"question": "Hidden", "response": "Wrong answer.", "condition": lambda: False},
        {"question": "Shown", "response": "Right answer."},
    ]
    npc = NPC("Guide", "A helpful guide.", dialogue_options)

    # call
    response = npc.get_dialogue_response(0)

    # check
    assert response == "Right answer."
